...
from algorithms.mst.kruskals.kruskal_mst import KruskalsMST
...
def run(sysargs):
    ...
    traverse_tree(KruskalsMST().run(Graph(graph_nodes[5])))
    traverse_tree(KruskalsMST().run(Graph(graph_nodes[0])))
    traverse_tree(KruskalsMST().run(Graph(graph_nodes[3])))
